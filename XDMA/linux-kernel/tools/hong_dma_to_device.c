		    ...
		    char *infname, char *ofname)
{
	...
	if (!underflow) {
        total_time = 1000000000*ts_end.tv_sec + ts_end.tv_nsec;
		avg_time = (float)total_time/(float)count;
        fprintf(stdout, "%s write() %ld bytes for %d times, total_time %ld nsec, avg_time %f nsec\n", 
        devname, bytes, count, total_time, avg_time);
	}
